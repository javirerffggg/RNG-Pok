...
import { ScanSearch, Sparkles, ChevronRight, Filter } from 'lucide-react'
...
export function TargetFinder() {
  ...
  const [targetAbility,   setAbility]        = useState<string>('')
  ...
  const handleSearch = () => {
    ...
    worker.onmessage = (e) => {
      if (e.data.type === 'progress') {
        ...
        // Enriquecer resultados con habilidad y género del Pokémon seleccionado
  ...
  // Filtrar por habilidad en el cliente (post-búsqueda)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card rounded-2xl p-4 space-y-4">
        ...
        {/* Pokémon picker */}
        <PokemonPicker value={pokemon} onChange={p => { setPokemon(p); setAbility('') }} label="Pokémon objetivo" genFilter={5} />
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {pokemon.abilities.map((a, i) => (
                <motion.button key={a} whileTap={{scale:0.93}}
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    targetAbility===a
                      ? 'text-white' : 'glass-card text-slate-400'
                  }`}
            ...
            <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}}
              ...
              <div className="flex items-center justify-between glass-card rounded-xl px-4 py-3">
                ...
                <motion.button onClick={() => setTargetShiny(p=>!p)}
                  ...
                  <motion.div animate={{ x: targetShiny ? 24 : 2 }}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {NATURES.map(n => {
                    ...
                    const cc = NATURE_STAT[n] || 'nature-neu'
                    return (
                      <motion.button key={n} whileTap={{scale:0.9}} onClick={() => toggleNature(n)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                          active ? `bg-poke-card border border-poke-accent/50 ${cc}` : 'glass-card text-slate-500 hover:text-slate-300'
                        }`}>{n}</motion.button>
                    )
                  })}
                </div>
          ...
          <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold">
                ...
                <span className="text-slate-400 ml-1.5">frames</span>
                {targetAbility && <span className="text-slate-500 text-xs ml-1">· {targetAbility}</span>}
              </p>
              {pokemon && (
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
              {displayed.slice(0,50).map((r, idx) => (
                <motion.div key={r.frame}
                  initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:idx*0.02}}
                  className={`rounded-xl p-3 ${r.shiny ? 'shiny-border' : 'glass-card'}`}>
